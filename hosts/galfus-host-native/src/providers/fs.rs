...
impl NativeFsProvider {
    ...
    fn extract_surface_path(args: &[SurfaceValue]) -> Result<String, ExecutionFailure> {
        match args.first() {
...
impl HostProvider for NativeFsProvider {
    ...
    fn dispatch_surface(
        ...
        let result = (|| -> Result<SurfaceValue, ExecutionFailure> {
            if name == "fs_normalize_path" {
                return Self::extract_surface_path(args)
                    .map(|path| SurfaceValue::Bytes(path.into_bytes()));
            }

            let path = Self::extract_surface_path(args)?;
            match name {
                "fs_read" => Ok(std::fs::read(&path)
                    .map(SurfaceValue::Bytes)
                    .unwrap_or(SurfaceValue::Null)),
                "fs_write" => {
                    let [SurfaceValue::Bytes(_), SurfaceValue::Bytes(data)] = args else {
