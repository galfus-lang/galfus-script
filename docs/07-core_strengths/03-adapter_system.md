...
While the *Provider System* handles robust native capabilities, the **Adapter System** is specifically designed for Galfus' flexible **Foreign Function Interface (FFI)**.

## Proxy Modules (`.gfp`)

Whenever the execution of a Galfus script needs to trigger arbitrary logic from an external application (a custom database, a game engine it's embedded in, or an AI API), it won't use loose `.gfs` code, but rather the declaration of a *Proxy*.
...
1. **Compilation and Validation**: The Workspace triggers a compatible `AdapterSchema` to ensure that the `.gfp` file is well-formed and semantically correct without running any *Host* logic.
