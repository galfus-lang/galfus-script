...
Your script code never runs directly. The Workspace builds and compiles the code into a rigorously validated *Bytecode* graph. Only if no business or memory rules are violated is the program released for execution on the Virtual Machine.

This means that, despite being a scripting language, Galfus brings the robustness of a compiled language into the embedding process. In the next step of your learning journey, you will discover how the ecosystem handles host capabilities through *Providers* and *Adapters*.
