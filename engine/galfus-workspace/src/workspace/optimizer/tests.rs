use super::*;
use galfus_bytecode::{
    BytecodeType, ConstIdx, ConstantPool, DebugLocation, ExecutionMetadata, FuncIdx, Reg, TypeIdx,
};
use galfus_contract::{ExecutionTarget, LimitsMetadata};
use galfus_core::{SemanticRevision, SourceId, Span};
use std::sync::Arc;
...
fn finalizing_an_already_canonical_package_reuses_its_allocation() {
    ...
    let finalized = optimize_package(Arc::clone(&package), SemanticRevision::new(1))
