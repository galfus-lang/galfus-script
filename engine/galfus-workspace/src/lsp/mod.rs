    ...
    fn dispatch_lsp_request(
        ...
        match method {
            ...
            "textDocument/definition" => {
                if let Some(p) = params
                    && let Ok(def_params) = serde_json::from_value::<GotoDefinitionParams>(p)
                {
                    let uri = &def_params.text_document_position_params.text_document.uri;
                    if let Some(path_str) = self.uri_to_module_path(uri)
                        && let Some(loc) = definition::goto_definition(
                            self,
                            &path_str,
            ...
            "textDocument/semanticTokens/full" => {
                if let Some(p) = params
                    && let Ok(st_params) = serde_json::from_value::<SemanticTokensParams>(p)
                {
                    let uri = &st_params.text_document.uri;
                    if let Some(path_str) = self.uri_to_module_path(uri)
                        && let Some(tokens) = semantic_tokens::semantic_tokens_full(self, &path_str)
                    {
                        return Some(JsonRpcResponse::success(
                            id,
            ...
            "galfus/virtualDocument" => {
                if let Some(p) = params
                    && let Some(uri_str) = p.get("uri").and_then(|v| v.as_str())
                    && let Ok(uri) = lsp_types::Url::parse(uri_str)
                {
                    let path = uri.path().trim_start_matches("/virtual/");
                    if let Some(module_path) = galfus_core::ModulePath::new(path)
                        && let Some(entry) = self.source_state.store.get(&module_path)
                    {
    ...
    fn dispatch_lsp_notification(
        ...
        match method {
            "textDocument/didOpen" => {
                if let Some(p) = params
                    && let Ok(open_params) = serde_json::from_value::<DidOpenTextDocumentParams>(p)
                {
                    let uri = &open_params.text_document.uri;
                    if let Some(path_str) = self.uri_to_module_path(uri)
                        && ModulePath::new(&path_str).is_some()
                    {
                        let _ =
                            self.load_module(&path_str, open_params.text_document.text.as_bytes());
