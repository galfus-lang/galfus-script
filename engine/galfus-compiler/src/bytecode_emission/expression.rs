        ...
        match rvalue {
            ...
            RValue::BinaryOp(op, lhs, rhs) => {
                ...
                let instr = if let Some((rhs, operation)) = immediate {
                    ...
                    self.emit_i32_binary_instruction(op, dest, lhs_reg, rhs_reg)
                } else if is_i64 {
                    self.emit_i64_binary_instruction(op, dest, lhs_reg, rhs_reg)
                } else if is_f32 {
                    self.emit_f32_binary_instruction(op, dest, lhs_reg, rhs_reg)
                } else if is_f64 {
                    self.emit_f64_binary_instruction(op, dest, lhs_reg, rhs_reg)
                } else {
                    match op {
