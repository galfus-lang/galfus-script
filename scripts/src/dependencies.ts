...
export async function checkCrateDependencies(): Promise<void> {
  const cargo = Bun.spawn(['cargo', 'metadata', '--format-version', '1', '--no-deps'], {
  ...
  const packages = new Map(
    (JSON.parse(output) as CargoMetadata).packages.map((pkg) => [pkg.name, pkg]),
  );
  const violations: string[] = [];

  for (const [packageName, forbiddenDependencies] of Object.entries(FORBIDDEN_DEPENDENCIES)) {
  ...
  if (violations.length > 0) {
    throw new Error(`Forbidden crate dependencies detected:\n- ${violations.join('\n- ')}`);
  }
}
