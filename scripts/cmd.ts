...
const setup = program
  .command("setup")
  .description("Local development setup commands");
const playground = program
  .command("playground")
  .description("Playground development and distribution commands");
const hosts = program
  .command("hosts")
  .description("Host binaries build and release commands");
const github = program
  .command("github")
  .description("GitHub Actions integration commands");
const check = program
  .command("check")
  .description("Repository validation commands");
...
dev
  .command("vscode")
  .description(
    "Build the local CLI and launch VS Code in extension development mode",
  )
  .action(devVscode);
...
dev
  .command("codemirror")
  .description(
    "Build and serve the WASM LSP wrapped in a local CodeMirror instance",
  )
...
playground
  ...
  .option(
    "-t, --target <target>",
    "wasm-bindgen target (web, bundler, nodejs, etc)",
    "web",
  )
  .option(
    "-o, --out-dir <path>",
...
hosts
  ...
  .option(
    "-p, --profile <profile>",
    "Build profile (debug, fastest, minimal)",
    "debug",
  )
...
github
  .command("set-version")
  .description(
    "Set workspace version and Github Action outputs based on the current branch",
  )
