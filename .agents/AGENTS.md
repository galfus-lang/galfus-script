...
- **Init indexing** (when `.codegraph/*.db` not exists): run `codegraph init` for initial indexing the codebase. If the command is not found, skip CodeGraph entirely — indexing is the user's decision.
- **MCP tool** (when available): `codegraph_explore` answers most code questions in one call — the relevant symbols' verbatim source plus the call paths between them, including dynamic-dispatch hops grep can't follow. Name a file or symbol in the query to read its current line-numbered source. If it's listed but deferred, load it by name via tool search. Can init MCP server with `codegraph serve --mcp` command.
